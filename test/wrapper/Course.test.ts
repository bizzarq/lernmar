...
test("course result in a nested course", async () => {
  ...
  expect(course1.courseState()).toMatchObject({progress: 0});
  expect(course2.courseState()).toMatchObject({progress: 0});
  expect(course3.courseState()).toMatchObject({progress: 0});
  expect(course.courseState()).toMatchObject({progress: 0});
      
  await course.executeActivity("course 1.activity 11");
  await course.executeActivity("course 1.activity 12");

  expect(course1.courseState()).toMatchObject({progress: 1, success: true, score: 160, maxScore: 200});
  expect(course.courseState()).toMatchObject({progress: 0.25, score: 160, maxScore: 200});

  await course.executeActivity("course 2.activity 21");
  await course.executeActivity("course 2.activity 22");

  expect(course2.courseState()).toMatchObject({progress: 1, success: false, score: 159, maxScore: 200});
  expect(course.courseState()).toMatchObject({progress: 0.5, score: 319, maxScore: 400});
